...
def vaultScreen():
    ...
    lbl.grid(column=0)
    tab1 = Frame(notebook)  # new fram for tab 1
    tab2 = Frame(notebook)
    notebook.add(tab1,text="KRONOS")
    notebook.add(tab2,text="BINANCE")
    notebook.grid()
    ...
    lbl = Label(tab2, text="Website")
    lbl.grid(row=2, column=0, padx=80)
    lbl = Label(tab2, text="Username")
    lbl.grid(row=2, column=1, padx=80)
    lbl = Label(tab2, text="Password")
    lbl.grid(row=2, column=2, padx=80)
    ...
    if (cursor.fetchall() != None):
        ...
        while True:
            ...
            lbl1 = Label(tab2, text=(decrypt(array[i][1], encryptionKey)), font=("Helvetica", 12))
            lbl1.grid(column=0, row=(i+3))
            lbl2 = Label(tab2, text=(decrypt(array[i][2], encryptionKey)), font=("Helvetica", 12))
            lbl2.grid(column=1, row=(i+3))
            lbl3 = Label(tab2, text=(decrypt(array[i][3], encryptionKey)), font=("Helvetica", 12))
            lbl3.grid(column=2, row=(i+3))

            btn = Button(tab2, text="Delete", command=  partial(removeEntry, array[i][0]))
            btn.grid(column=3, row=(i+3), pady=10)
